...
if __name__ == "__main__":
    ...
    net_ig = Generator6(ngf=64, nz=noise_dim, nc=3, im_size=args.im_size)#, big=args.big )
    net_ig.to(device)
    ...
    for epoch in [5000*i for i in range(args.start_iter, args.end_iter+1)]:
        ...
        dist = os.path.join(args.artifacts, 'eval_%d'%(epoch))
        ...
        with torch.no_grad():
            for i in tqdm(range(args.n_sample//args.batch)):
                ...
                g_imgs = net_ig(noise, skips=True)
